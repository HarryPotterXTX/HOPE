...
def plot(coefs, ref_coef, title, xlabels):
    ...
    w, h = 0.64, 0.48
    plt.figure(figsize=(w*ref_coef.shape[0], h*ref_coef.shape[0]))
...
def main(net_path, order, point, num, coord_range, top):
    ...
    # show the results
    plot(coefs, ref_coef, title=f'Coefficients on {point[0]}', xlabels=get_interactions(p, order))
    ...
    plot(top_mean_coef, top_ref_coef, title=f'Top {top} coefficients on {point[0]}', xlabels=xlabels)
